...
\frame{
	...
	%\vspace{-3.5pt}
	\begin{itemize}
		\item All eigenvalues $\mu$ of $B$ not $\pm 1$ are the roots of the equation\footnote[frame]{Omer Angel, Joel Friedman, and Shlomo Hoory (2015). The non-backtracking spectrum of the universal cover of a graph. Trans. Amer. Math. Soc., 367(6):4287-4318.}
		\vspace{-10pt}
		\begin{equation} 
			\label{eq:ihara}
...
\frame{
	...
	\vspace{-9pt}
	\begin{itemize}
		\item All eigenvalues $\mu$ of $B$ not $\pm 1$ are the roots of the equation
		\vspace{-10pt}
		\begin{equation} 
			\label{eq:ihara}
			\det \left[ \mu^2 \id - \mu A + (D-\id) \right] = 0 \,
...
\frame{
	...
	\begin{itemize}
		\item Comparing to non-backtracking clustering
			\begin{itemize}
				\item $H(r_c)$ is $n\times n$ symmetric, whereas $B'$ is $2n\times 2n$ non-symmetric
				\item Need to compute $\rho(B)$ to calculate $r_c =$
			\end{itemize}
	\end{itemize}
}
